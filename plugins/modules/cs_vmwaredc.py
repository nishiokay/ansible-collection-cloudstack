...
DOCUMENTATION = '''
---
module: cs_storage_network_ip_range
...
- name: create a VLAN IP range for network test
  ngine_io.cloudstack.cs_storage_network_ip_range:
    network: test
    vlan: 98
    start_ip: 10.2.4.10
    end_ip: 10.2.4.100
...
class AnsibleCloudStackStorageVlanIpRange(AnsibleCloudStack):

    def __init__(self, module):
        super(AnsibleCloudStackStorageVlanIpRange, self).__init__(module)
        self.returns = {
            'startip': 'start_ip',
            'endip': 'end_ip',
            'podid': 'pod_id',
    ...
    def get_storage_network_ip_range(self):
        if not self.ip_range:
            ...
            res = self.query_api('listStorageNetworkIpRange', **args)
            if res:
                ip_range_list = res['storagenetworkiprange']
    ...
    def present_storage_network_ip_range(self):
        ip_range = self.get_storage_network_ip_range()

        if not ip_range:
            ip_range = self.create_storage_network_ip_range()

        return ip_range

    def create_storage_network_ip_range(self):
        ...
        if not self.module.check_mode:
            res = self.query_api('createStorageNetworkIpRange', **args)

            poll_async = self.module.params.get('poll_async')
            if poll_async:
                ip_range = self.poll_job(res, 'storagenetworkiprange')

        return ip_range

    def absent_storage_network_ip_range(self):
        ip_range = self.get_storage_network_ip_range()
...
def main():
    ...
    acs_storage_network_ip_range = AnsibleCloudStackStorageVlanIpRange(module)

    state = module.params.get('state')
    if state == 'absent':
        ipr = acs_storage_network_ip_range.absent_storage_network_ip_range()

    else:
        ipr = acs_storage_network_ip_range.present_storage_network_ip_range()

    result = acs_storage_network_ip_range.get_result(ipr)
